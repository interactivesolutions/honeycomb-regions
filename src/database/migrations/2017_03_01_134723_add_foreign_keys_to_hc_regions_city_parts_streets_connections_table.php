...
class AddForeignKeysToHcRegionsCityPartsStreetsConnectionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('hc_regions_city_parts_streets_connections', function(Blueprint $table) {
            $table->foreign('city_part_id',
                'fk_hc_regions_city_parts_streets_connections_hc_regions_city_1')->references('id')->on('hc_regions_city_parts')->onUpdate('NO ACTION')->onDelete('NO ACTION');
            $table->foreign('street_id',
                'fk_hc_regions_city_parts_streets_connections_hc_regions_stree1')->references('id')->on('hc_regions_streets')->onUpdate('NO ACTION')->onDelete('NO ACTION');
        });
    }
